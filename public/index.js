...
const heygen_API = {
  apiKey: 'NDYxOTc5OTk0ZjI5NDMzNzgyNjNmZTdjYmExMjE5ZmYtMTcxNzU3OTc2Ng==',
...
//Talk to llm
async function talkToOpenAI(question) {
  console.log('Talking to LLM with question:', question);

  const url = 'http://172.20.10.2:5003/qad'; 
  const data = {
    question: question,
    username: 'ajay', 
    language: 'english', 
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Failed to fetch from LLM:', response.statusText);
